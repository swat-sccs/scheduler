#!/bin/sh
pdftotext -x 33 -y 60 -W 723 -H 513 -bbox ../course_schedule_current.pdf all.txt
# can use any excel-like spreadsheet (delimiter is "|" (without quotes)) or use linux CLI column `column -ts "|" |less -S` makes a nice looking plain text spreadsheet for inspection.
./pdfToCSV.js > schedule.csv
