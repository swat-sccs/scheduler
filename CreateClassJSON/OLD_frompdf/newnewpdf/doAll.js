...
fs.readFile("./all.txt", "utf8", function(err, results){
    ...
    for(var i = 0; i<results.length;i++){
        ...
        if(prevY.indexOf(y) == -1){
            prevY.push(y);
            pages[lastPgIndex].push({y: y, crs:"",subj:"",num:"",sec:"",title:"",cred:"",dist:"",lim:"",instruc:"",type:"",days:"",times:"",bld:"", notes: ""});
        }
        ...
        if(x >=33 && x<=85){
            pages[lastPgIndex][yIndex].crs += parsedLine[5]+" ";
        }else if(x >=85 && x<=112){
            pages[lastPgIndex][yIndex].subj += parsedLine[5]+" ";
        }else if(x >=112 && x<=142){
            pages[lastPgIndex][yIndex].num += parsedLine[5]+" ";
        } else if(x >=142 && x<=172){
            pages[lastPgIndex][yIndex].sec += parsedLine[5]+" ";
        } else if(x >=172 && x<=293){
            pages[lastPgIndex][yIndex].title += parsedLine[5]+" ";
        } else if(x >=293 && x<=322){
            pages[lastPgIndex][yIndex].cred += parsedLine[5]+" ";
        } else if(x >=322 && x<=357){
            pages[lastPgIndex][yIndex].dist += parsedLine[5]+" ";
        } else if(x >=357 && x<=409){
            pages[lastPgIndex][yIndex].lim += parsedLine[5]+" ";
        } else if(x >=409 && x<=509){
            pages[lastPgIndex][yIndex].instruc += parsedLine[5]+" ";
        } else if(x >=509 && x<=579){
            pages[lastPgIndex][yIndex].type += parsedLine[5]+" ";
        } else if(x >=579 && x<=609){
            pages[lastPgIndex][yIndex].days += parsedLine[5]+" ";
        } else if(x >=609 && x<=689){
            pages[lastPgIndex][yIndex].times += parsedLine[5]+" ";
        } else if(x >=689 && x<=759){
            pages[lastPgIndex][yIndex].bld += parsedLine[5]+" ";
        }
    ...
    for(var q = 0; q<pages.length;q++){
        for(var z=pages[q].length-1; z>=0;z--){
            ...
            if(pages[q][z].crs == "" && pages[q][z].type!=""){
                //doesn't go to instruc
                pages[q][z-1].instruc+=" & "+pages[q][z].instruc;
                deleteLine = true;
            }
    ...
    for(var q = 0; q<pages.length;q++){
        for(var z in pages[q]){
            ...
            console.log(pages[q][z].crs+"|"+pages[q][z].subj+"|"+pages[q][z].num+"|"+pages[q][z].sec+"|"+pages[q][z].title+"|"+pages[q][z].cred+"|"+pages[q][z].dist+"|"+pages[q][z].lim+"|"+pages[q][z].instruc+"|"+pages[q][z].type+"|"+pages[q][z].days+"|"+pages[q][z].times+"|"+pages[q][z].bld+"|$|"+pages[q][z].notes);
        }
    }
